      ...
      <div class="flex-auto">
        <div class="container mx-auto md:px-5 md:pb-5">
          ...
          <div class="block md:flex">
            <div class="flex-auto">
              <div class="w-full">
                <div class="mb-2 flex rounded-lg border-2 border-fuchsia-600">
                  <input
          ...
          <div class="mt-10 block text-gray-500" :update="update">
            <div v-if="translates.result.length" class="mb-5 text-gray-800">
              ...
              <p v-for="(result, i) in translates.result" :key="result.id">
                <span>
                  ...
                  <br />
                  <span v-for="translation in result.translations">
                    {{ translation.name }}
                    <br />
                  </span>
                </span>
          ...
          <div class="mt-5 block text-gray-700">
            <aside
              id="revue-embed"
          ...
          <div class="mt-5 block text-gray-700" v-if="messageFromServer.show">
            <h2 class="py-1 text-xl sm:text-xl lg:text-6xl">
              {{ messageFromServer.body.title }}
            </h2>
            <p>
              {{ messageFromServer.body.message }}
            </p>
          </div>
          <div class="mt-5 block text-gray-500">
            <p>* {{ $t('disclaimer') }}</p>
          </div>
          <div class="mt-5 block text-gray-500">
            <p>
          ...
          <div class="mt-5 block pb-2 text-gray-500">
            <p>
              У нас также есть мобильные приложения и расширение для браузера
              Google Chrome:
            </p>
          </div>
          <div class="mt-5 block pb-2 text-gray-500">
          ...
          <div class="flex justify-center">
            <div class="mt-5 block justify-center pb-10 text-gray-500 md:flex">
              <a
  ...
  export default Vue.extend({
    ...
    methods: {
      ...
      async initDailyTranslationsCount() {
        this.dailyTranslationsCount =
          (await this.$axios.$get('/api/statistic/daily-translations-count'))
            ?.count || 0;
      },
      initTextAndLanguageConfigurations() {
        const {
          s: sourceLanguage,
          d: destinationLanguage,
          t: translateWord,
        } = this.$route.query;
        if (
          sourceLanguage &&
      ...
      getLinkToEditForm(word) {
        return this.localePath(
          `/admin/words/${this.sourceLanguage}/${this.destinationLanguage}/${word.id}`,
        );
      },
      ...
      async translate(e: any, imidiately: boolean = false): any {
        ...
        this.translateTimeout = setTimeout(
          async () => {
            ...
            try {
              this.translates = await this.translateText(
                this.sourceLanguage,
              ...
              const translationResultCount = this.countResultCount(
                this.translates,
              );
              if (translationResultCount === 0) {
                const oppositeTranslates = await this.translateText(
                  this.destinationLanguage,
                  this.sourceLanguage,
                  value,
                );
                this.foundWordsInOppositeLanuage =
                  this.countResultCount(oppositeTranslates);
              }
            } catch (e) {
              console.error(e);
            } finally {
      ...
      countResultCount(translates) {
        return translates.result.filter(
      ...
      async translateText(sourceLanguge, destinationLanguage, text) {
        const {
          data: { data },
        }: any = await this.$axios.get(
          `/api/translate/${sourceLanguge}2${destinationLanguage}?` +
            this.jsonObjectToQueryString({
              word: text,
            }),
        );
        return data;
      ...
      jsonObjectToQueryString(
        ...
        const isNotNullObject = (v: object) =>
