...
export default Vue.extend({
  ...
  async mounted() {
    if (this.wordId) {
      this.word = await this.$axios.$get(`/api/admin/words-matcher/${this.sourceLanguageCodeCode}/${this.destinationLanguageCode}/${this.wordId}`);
    }
    console.log(this.$route.query.word);
    if (this.$route.query?.word) {
      this.word.name = this.$route.query?.word;
      try {
        this.$axios.$post(`/api/admin/words-matcher/${this.sourceLanguageCodeCode}/word`, this.word).then(word => {
  ...
  methods: {
    ...
    async loadWord(sourceLanguageCodeCode, destinationLanguageCode, wordId) {
      return await this.$axios.$get(`/api/admin/words-matcher/${sourceLanguageCodeCode}/${destinationLanguageCode}/${wordId}`) || this.defaultWord();
    },
    async loadSimilarWords(word: string) {
        this.suggestedWords = await this.$axios.$get(`/api/admin/words-matcher/${this.sourceLanguageCodeCode}/${this.destinationLanguageCode}?word=${word}`);
    },
    ...
    async saveWord(word) {
      return await this.$axios.$post(`/api/admin/words-matcher/${this.destinationLanguageCode}/${this.sourceLanguageCodeCode}`, word);
    },
    async sync(word) {
      return await this.$axios.$put(`/api/admin/words-matcher/${this.sourceLanguageCodeCode}/${this.destinationLanguageCode}/${word.id}/sync`, word);
    },
    async save() {
